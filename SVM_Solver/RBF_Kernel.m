...
clc;
C = [2 2 5 5 10 10 100 100];
gama = [1 2 1 2 1 2 1 2];
for k = 1:8;
...
xlabel('x1');
ylabel('x2');
str = ['C=', num2str(C(k)),'  gama=', num2str(gama(k))];
title(str);
%quadratic Kernel
Kq = zeros(30,30); %30*30
for i = 1:30
    for j=1:30
        Kq(i,j)=exp(-gama(k)*(norm((xi(:,i)-xi(:,j))'))^2);
    end
end
...
res.sol.itr.xx
elfa = res.sol.itr.xx';
index1 = find( res.sol.itr.xx > 0.0001 );
index2 = find( res.sol.itr.xx < C(k)-0.0001 );
index = intersect(index1,index2);
scatter(xi(1,index), xi(2,index),'h','r','filled');
b = yi(1,index(1)) - (elfa.*yi)*((exp(-gama(k)*((x_1-xi(1,index(1))).^2+(x_2-xi(2,index(1))).^2)))');
x1 = -1:0.01:1;
x2 = -1:0.01:1;
para = elfa.*yi;
[x1,x2] = meshgrid(x1,x2);
func = @(x,y) para*(exp(-gama(k)*((x-x_1).^2+(y-x_2).^2)))' + b;
...
y_new = ones(1,5);
for i = 1:5
    if para*(exp(-gama(k)*((x_new(1,i)-x_1).^2+(x_new(2,i)-x_2).^2)))' + b > 0
        y_new(1,i) = 1;
    else
        y_new(1,i) = -1;
    end

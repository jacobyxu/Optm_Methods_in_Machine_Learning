...
KQ = yij.*Kq;
KQ = KQ + 10^(-7)*eye(length(y),length(y));
q     = KQ; 
...
elfa = res.sol.itr.xx';
index1 = find( res.sol.itr.xx > 10^-7  );
index2 = find( res.sol.itr.xx < C(k)-10^-7  );

index = intersect(index1,index2);
b = y(index(1),1) - (elfa.*(y)')*(x*((x(index(1),:))' ));
% Test
para = elfa.*(y');
testh = zeros(length(newy),1);
for i = 1:length(newy)
    testh(i,1)= para*((x*((newx(i,:))')) ) + b;
end
testhy = newy.*testh;
